...
def calculate_metrics(gt_file, pred_file, voxel_sz):
    ...
    # gt_array_L = gt_array
    # pred_array_L = pred_array
    # gt_array_L[gt_array_L == 1] = 0
    # pred_array_L[pred_array_L == 1] = 0
    
    # gt_array_R = pred_array
    # pred_array_R = pred_array
    # gt_array_R [gt_array_R == 2] = 0
    # pred_array_R[pred_array_R == 2] = 0
...
def score_seg(gold, pred_lst, label):
    ...
    scores = []
    voxel_sz = [1, 1, 1]  # Example voxel size, replace with actual if different
    ...
    for pred in pred_lst:
        ...
        parts = gold[ind].split('_') #needs to be changes regarding the new naming of the prediction and goldstandard files.
